function ColOBB(position, size, orientation) constructor {
	...
	static DebugDraw = function() {
	    static draw_vertex_add_point = function(vbuff, x, y, z, colour=c_red) {
	    ...
	    // Draw vertices of the OBB
	    for (var i = 0; i < 8; i++) {
	        draw_vertex_add_point(vbuff, vertices[i].x, vertices[i].y, vertices[i].z, c_red);
	    }
    
	    // Draw edges of the OBB (12 edges in total)
	    var edges = self.GetEdges();
    
	    for (var i = 0; i < array_length(edges); i++) {
	        var edge = edges[i];
	        draw_vertex_add_point(vbuff, edge.start.x, edge.start.y, edge.start.z, c_red);
	        draw_vertex_add_point(vbuff, edge.finish.x, edge.finish.y, edge.finish.z, c_red);
	    }
